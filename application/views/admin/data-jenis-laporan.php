<?php
// Prefix halaman ini
$prefix_page = 'admin/kelola/jenis_laporan/';
              ...
              <div class="card-body table-responsive">
                ...
                <div class="">
                  <table id="datatable" class="table dt-responsive table-hover nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                      <?php $no = 1; foreach ($jenis_laporan as $val): ?>
                      <tr>
                        <td><?=$no++ ?></td>
                        <td><?=$val->nama_laporan ?></td>
                        <td>
                          <center>
                            <button type="button" class="btn btn-sm btn-success mr-2 edit-table" data-id="<?=$val->id ?>" data-nama="<?=$val->nama_laporan ?>"><i class="fas fa-edit"></i></button>
                            <button type="button" class="btn btn-sm btn-danger delete-table" data-id="<?=$val->id ?>" data-nama="<?=$val->nama_laporan ?>"><i class="fas fa-trash-alt"></i></button>
                          </center>
                        </td>
                      </tr>
                      <?php endforeach; ?>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('.tambah-data').on('click', function(e) {
      ...
      $('.custom-modal-title').html('Tambah Data Jenis Laporan');
    ...
    $('.edit-table').on('click', function(e) {
      ...
      $('.custom-modal-title').html('Edit Data Jenis Laporan');
      $('#form').attr('action', url)
      $('[name=id]').val(id)
      $('[name=nama_laporan]').val(nama)
      modalku('#custom-modal', 'fadein');
    })
      ...
      <div class="col">
        <form class="form" id="form" action="" method="post">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" required placeholder="Isi nama jenis_laporan" name="nama_laporan" />
          </div>
